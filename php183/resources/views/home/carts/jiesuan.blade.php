...
<form action="{{ url('/home/orders') }}" method="post">
...
<div class="box-01">
    ...
    <span>{{ $dizhi->addinfo }}</span>
    <span>{{ $dizhi->email }}</span>
    @endforeach
    <input type="hidden" id="chooseid-SH"> 
    </div>
    <div id="bg-SH" class="bg-n"></div>
    <div style="display:none" id ="addAddr" class="box-SH box">
    <input type="hidden" id="flag1-SH" value="0">
    <input type="hidden" id="deliveridupdate-SH" value=""><a class="close" onclick="closes()" href="javascript:void(0);" >╳</a>
    <ul><li><label><span>* </span>收货人：</label><input type="text" id="name-SH">
    <label id="alertName-SH" style="display:none;color:red">收货人不能为空</label>
    	...
    	<select id="goodsCounty-SH" name="goodsCounty-SH" style=" height:25px;width:120px"><option value="">区/县</option></select></li>
        <li><label><span>* </span>详细地址：</label><input type="text" style="width:410px;" id="goodsStreet-SH"></li>
        <li><label>邮箱：</label><input type="text" id="mail-SH">
...
<input type="hidden" name="ids[]" value="{{ $shop->sid }}" id="">
<input type="hidden" name="nums[]" value="{{ $shop->count }}" id="">
<input type="hidden" name="prices[]" value="{{ $shop->price }}" id="">
    <p class="list-a">
    <a title="{{ $shop->name }}" href="{{ url('/home/detail') }}/{{ $shop->sid }}">
    <img title="{{ $shop->name }}" width="110px" height="110px" src="/uploads/shopping/{{$shop->avatar}}"></a>
</p>
<p class="list-b">
    <a title="{{ $shop->name }}" href="{{ url('/home/detail') }}/{{ $shop->sid }}">{{ $shop->name }}</a>
</p>
    <p class="list-c">{{ $shop->price }}</p>
<p class="list-d">{{ $shop->count }}</p>
