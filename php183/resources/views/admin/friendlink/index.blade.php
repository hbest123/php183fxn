            ...
            @endif
              	<table id="example2" class="table table-bordered table-hover">
		            ...
		            <tbody>
                      ...
                      <td class="advername">{{ $val->advername }}</td>
                      <td class="address">{{ $val->address }}</td>
		                	<td><img style="width:40px;height:40px;" src="/uploads/shopping/{{ $val->avatar }}"></td>
		                	<td><a href="{{ url('/admin/friendlink/') }}/{{ $val->id }}/edit">编辑</a>
                        <a class="del" href="#">删除</a>
                      </td>
                      <form style="display:none" action="{{ url('/admin/friendlink') }}/{{ $val->id }}" method="post">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                      </form>
